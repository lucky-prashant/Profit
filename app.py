...
def fetch_candles(pair, count=30):
    try:
        ...
        data = r.json()
        candles = data.get("values", [])[::-1]  # Latest first
        return candles
...
def analyze_cwrv(candles):
    try:
        ...
        return {
            ...
            "candle": {"open": C3["open"], "close": C3["close"], "high": C3["high"], "low": C3["low"]}
        }
    except Exception as e:
        return {
            "direction": "N/A",
    ...
    for pair in PAIRS:
        try:
            if pair not in candle_cache:
                ...
                last_timestamps[pair] = candles[-1]["datetime"]
            else:
                new_candle = fetch_candles(pair, 1)
                if new_candle:
                    last_time = last_timestamps.get(pair)
                    if new_candle[-1]["datetime"] != last_time:
                        candle_cache[pair].append(new_candle[-1])
                        last_timestamps[pair] = new_candle[-1]["datetime"]
            ...
            actual = "CALL" if float(candles[-1]["close"]) > float(candles[-1]["open"]) else "PUT"
